...
return_codes = {
    'Success': 0,
    'Error fetching data from the websites': -1,
    'stocks.json file not found': -2,
}


stocks_data_template = {
    'gold': {
        'urls': [
            "https://pricegold.net/ar/kw-kuwait/",
    ...
    'silver': {
        'urls': [
            "https://www.livepriceofgold.com/silver-price/kuwait.html",
...
def get_stocks_data():
    ...
    try:
        with open('stocks.json', 'r') as f:
            ...
            for stock_type, stock_data in stocks_data.items():
        ...
        # make a deep copy of the stocks_data_template dict
        stocks_data = json.loads(json.dumps(stocks_data_template))
...
async def progress_bar():
    ...
    with tqdm(total=100, desc=tdqm_message,
        ...
        pbar.set_description_str(f'\033[94m{tdqm_message}\033[0m')
...
def update_stock_price_json_file(stocks_data, stocks_prices):
    """
    ...
    print(f'\nUpdating the {file_name} file ....... ')

    if os.path.exists(file_name):
        with open(file_name, 'r') as file:
            temp_stocks_data = json.load(file)

            for stock_type, stock_average_price in stocks_prices:
                # get the previous price from the stocks_data dict
                _temp_stock_prices = stocks_data[stock_type]['prices']
                new_update = {
                    'date': datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                    'price': stock_average_price
                }
                # update the stocks_data dict with the new price
                _temp_stock_prices.append(new_update)

                # stocks_data[str(stock_type)]['price'] = _temp_stock_prices

                # update the function name for the stock type
                stocks_data[stock_type]['get_price_function'] = temp_stocks_data[stock_type]['get_price_function']

                # check and the status of the price either it is the same or changed
                last_price = stocks_data[stock_type]['prices'][-2]['price']
                ...
                if last_price > current_price:
                    ...
                    print(f'The {stock_type} price is not changed')
        ...
        with open(file_name, 'w') as file:
            for stock_type, stock_average_price in stocks_prices:
                ...
                # update the function name for the stock type
                stocks_data[stock_type]['get_price_function'] = stocks_data_template[stock_type]['get_price_function']

                # update the stocks_data dict with the new price
                stocks_data[stock_type]['prices'].append(new_update)

            json.dump(stocks_data, file, indent=4)
            print(f'The {file_name} file is created')
